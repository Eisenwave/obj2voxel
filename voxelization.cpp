 ...
 */
void splitTriangles(std::vector<TexturedTriangle> *preSplitBuffer,
                   ...
                   std::vector<TexturedTriangle> *resultBuffer)
{
    ...
    std::vector<TexturedTriangle> *const originalResultBuffer = resultBuffer;
    ...
    for (unsigned axis = 0; axis < 3; ++axis) {
        do {
            while (not preSplitBuffer->empty()) {
                ...
                // Elevate the splitting plane if the minimum lies on it.
                // If we don't do this, splitTriangle() might insert into postCut with no changes and we enter an
                // infinite loop.
                splittingPlane += eq(min, splittingPlane);

                splitTriangle(axis, splittingPlane, triangle, *resultBuffer, *postSplitBuffer);
            }
        ...
        if (postSplitBuffer != originalPostSplitBuffer) {
            std::swap(preSplitBuffer, postSplitBuffer);
        }
    ...
    // swap an even amount of times to restore original state
    std::swap(preSplitBuffer, resultBuffer);

    VXIO_DEBUG_ASSERT_EQ(preSplitBuffer, originalPreSplitBuffer);
    VXIO_DEBUG_ASSERT_EQ(postSplitBuffer, originalPostSplitBuffer);
    VXIO_DEBUG_ASSERT_EQ(resultBuffer, originalResultBuffer);
}
#endif
